...
import sortDesc from './../../assets/sort_desc.svg'
import sortAsc from './../../assets/sort_asc.svg'
...
const List = () => {
  ...
  const iconSort = () => {
    if (list.sort === 'DESC') {
      return <img src={sortDesc} />
    } else {
      return <img src={sortAsc} />
    }
  }
            ...
            <tbody>
              {list.filterList.map((item, index) => (
                <tr key={`${item.author}-${item.tech}`}>
                  <td>{star(`${item.author}-${item.tech}`)}</td>
                  <td>
              ...
              {list.filterList.length === 0 && (
                <tr>
                  <td colSpan="7">
                    No se encontrarón tecnologías con los parametros de búsqueda
                    especificados
                  </td>
                </tr>
              )}
