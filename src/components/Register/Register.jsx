...
const Register = () => {
  ...
  const getMessage = (object) => {
    ...
    if (object?.type === "maxLength") {
        return <span>El campo no puede exceder los {MAX_LENGTH_INPUT} caracteres</span>;
    }
    ...
    <div id="register">
      <div className="contentForm">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="content-input">
            <label>Pais</label>
            <select name="country" ref={register({ required: true })} onChange={getProvince} defaultValue={COUNTRY_INIT}>
          ...
          <div className="content-input">
            ...
            <select name="province" ref={register({ required: true })}>
                {countryInfo.provinces.map((province, index) =>
                    <option key={index} value={province.id}>{province.province}</option>
                )}
            </select>
          ...
          <div className="content-input">
            <label className='inline'>
                ...
                ref={register({required: true})}
                />
            Acepto los <a href='/TermsAndConditions' target='_blank'>Terminos y condiciones</a></label>
            <div>{getMessage(errors.terms)}</div>
          </div>

          <input type="submit" value='REGISTRARME' className='btn-send' disabled={!formState.isValid} />
        </form>
      </div>
    </div>
  );
